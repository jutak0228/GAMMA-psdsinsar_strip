...
# Part 4: Prepare DEM and geocode reference
...
cd ${work_dir}/input_rslc

# multilook reference
multi_look ${ref_date}.rslc ${ref_date}.rslc.par ${ref_date}.rmli ${ref_date}.rmli.par ${rlks} ${azlks}

# estimate corber latitude and longitude
if [ -e SLC_corners.txt ]; then rm -f SLC_corners.txt; fi
SLC_corners ${ref_date}.rmli.par > SLC_corners.txt
...
/bin/cp ../input_rslc/${ref_date}.rmli .
/bin/cp ../input_rslc/${ref_date}.rmli.par .
...
geocoding.py ${ref_date}.rmli ${ref_date}.rmli.par SRTM1.dem SRTM1.dem_par $ref_date --kml --seg ${dem_name} --thres 0.3

# # generate look-up table using gc_map2
# gc_map2 ${ref_date}.rmli.par ${dem_name}.dem_par ${dem_name}.dem ${ref_date}_seg.dem_par ${ref_date}_seg.dem ${ref_date}.lt 2 2 ${ref_date}.ls_map - ${ref_date}.inc
...
# pixel_area ${ref_date}.rmli.par ${ref_date}_seg.dem_par ${ref_date}_seg.dem ${ref_date}.lt ${ref_date}.ls_map ${ref_date}.inc ${ref_date}.sigma0 ${ref_date}.gamma0 20 0.01

# # calculate offset between gamma0 and multilooked image
# create_diff_par ${ref_date}.rmli.par - ${ref_date}.diff_par 1 0
# offset_pwrm ${ref_date}.gamma0 ${ref_date}.rmli ${ref_date}.diff_par ${ref_date}.offs ${ref_date}.snr 128 128 - 1 32 32 0.3
...
# pixel_area ${ref_date}.rmli.par ${ref_date}_seg.dem_par ${ref_date}_seg.dem ${ref_date}.lt_fine ${ref_date}.ls_map ${ref_date}.inc ${ref_date}.sigma0 ${ref_date}.gamma0 20 0.01
...
# range_pixel_spacing=`cat ${ref_date}.rmli.par   | grep "range_pixel_spacing"   | awk -F":" '{print $2}' | tr -d [:space:] | sed -e "s/[^0-9\.]//g"`
...
# geocode_back ${ref_date}.rmli ${range_samples} ${ref_date}.lt_fine ${ref_date}.geo ${ortho_width} ${ortho_nlines} 5 0 - - 3
